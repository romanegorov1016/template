...
const fileHelper = require('ngpd-merceros-testautomation-wdio-ta/helpers/file-helper');
...
exports.common_config = {
  ...
  before() {
    try {
      ...
      const localizationData = fs.readFileSync('./localization-storage.json');
      global.uniqueMap = JSON.parse(data);
      global.localizationMap = JSON.parse(localizationData);
    } catch (e) {
      global.uniqueMap = {};
      global.localizationMap = {};
    }
  },
  ...
  afterScenario() {
    if (this.params.restartBrowserBetweenTests) {
      ...
      try {
        await fileHelper.removeDirectory(downloadDir);
      } catch(err){
        console.log(`Directory ${downloadDir} is not exist`);
      }
    }
  },

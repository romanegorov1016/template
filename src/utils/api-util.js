...
const getValueOfPropertyFromJson = (object, jsonPathQuery) => {

  return jsonPath.value(object, jsonPathQuery);
};

const executeJsonPathQuery = (object, jsonPathQuery) => {

  return jsonPath.query(object, jsonPathQuery);
};

const getDeepDiffErrMsg = (deepDiffErr) => {
  if (typeof deepDiffErr === 'undefined' || deepDiffErr.length < 0) {
    return deepDiffErr;
  } else {
    const mapDeepDiffErrors = (error) => {
      let errStr;
      switch (error['kind']) {
        case 'N':
    ...
    const detailedDeepDiffErrors = deepDiffErr.map(err => { return '\n' + mapDeepDiffErrors(err); });

    return detailedDeepDiffErrors;
  }
};

const getFileDetailsForUpload = (filename, dir = dataStoreHelper.getData('testDataDir')) => {
...
const replaceValuesFromDataTable = (object, replacementDataTable) => {
  const replacementKeyValuePairs = replacementDataTable.raw();

  replacementKeyValuePairs.forEach(keyValuePair => {
    const propertyPath = keyValuePair[0];
    const newPropertyValue = parseValueFromString(keyValuePair[1]);

    replaceValueInProperty(object, propertyPath, newPropertyValue);
  });

  return object;
};

const getAuthInfoFromUniqueMap = () => {
  let authInfo = {
    headers: new Map(),
    jar: null
  };
  ...
  if (dataStoreHelper.getData('authInfo')) {
    // authInfo = JSON.parse(JSON.stringify(dataStoreHelper.getData('authInfo')));
 ...
 * @param shouldBe String value that could be equal to "be equal to" or "contain" for comparing
 */
const dataComparator = (v1, v2, shouldBe) => {
  switch (shouldBe) {
    case 'equal to':
      if (typeof v2 === 'object') {
        const diffRes = diff(v2, v1);

        return expect(v1, `${getDeepDiffErrMsg(diffRes)}`).to.deep.equal(v2);
      } else {

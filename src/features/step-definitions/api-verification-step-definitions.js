...
const { dataStoreHelper } = require('ngpd-merceros-testautomation-wdio-ta');
...
Then('Response {text} {text} in {text} is {text} {text}', (every, property, responseElement, expected, value) => {
  ...
  if (typeof value === 'string') {
    value = utils.parseValueFromString(value);
  }
  ...
  if (every.toLowerCase() === 'every property') {
    ...
    responseProperty.forEach((data, index) => {
      try {
        utils.dataComparator(data[property], value, expected);
      }
      catch (e) {
        errMsg.push(`\n[${index}]: ${e.message}`);
      }
    ...
    try {
      utils.dataComparator(responseProperty[property], value, expected);
    }
    catch (e) {
...
Then('User verifies response with jsonPath {string} result is equal to {text}', async (pathQuery, expectedValue) => {
  const response = dataStoreHelper.getData('response');
  let result = utils.executeJsonPathQuery(response, pathQuery);

  if (typeof expectedValue === 'string') {
    expectedValue = utils.parseValueFromString(expectedValue);
  }
...
Then('User verifies response with jsonPath {string} result contains {text}', async (pathQuery, expectedValue) => {
  ...
  return expect(utils.executeJsonPathQuery(response, pathQuery)).to.contains(expectedValue);
});
...
Then('User verifies response with jsonPath {string} result does not contain {text}', async (pathQuery, expectedValue) => {
  ...
  return expect(utils.executeJsonPathQuery(response, pathQuery)).to.not.contains(expectedValue);
});
 ...
 * path query examples: https://www.npmjs.com/package/jsonpath
 */
Then('User verifies response contains property {text} with value {text}', (propertyName, expectedValue) => {
  const pathQuery = `$..[?(@.${propertyName}=='${expectedValue}')]`;
  const response = dataStoreHelper.getData('response');

  return expect(utils.getValueOfPropertyFromJson(response, pathQuery), `Response does not contain expected value: ${expectedValue}`).not.to.be.undefined;
});
 ...
 * path query examples: https://www.npmjs.com/package/jsonpath
 */
Then('User verifies response does not contain property {text} with value {text}', (propertyName, expectedValue) => {
  const pathQuery = `$..[?(@.${propertyName}=='${expectedValue}')]`;
  const response = dataStoreHelper.getData('response');

  return expect(utils.getValueOfPropertyFromJson(response, pathQuery), `Response contains value: ${expectedValue}`).to.be.undefined;
});
 ...
 * path query examples: https://www.npmjs.com/package/jsonpath
 */
Then('User verifies response contains property {text} in {text} with value {text}', (propertyName, responseElement, expectedValue) => {
  const pathQuery = `$..${responseElement}[?(@.${propertyName}=='${expectedValue}')]`;
  const response = dataStoreHelper.getData('response');

  return expect(utils.getValueOfPropertyFromJson(response, pathQuery), `Response does not expected value: ${expectedValue}`).not.to.be.undefined;
});
 ...
 * path query examples: https://www.npmjs.com/package/jsonpath
 */
Then('User verifies response does not contain property {text} in {text} with value {text}', (propertyName, responseElement, expectedValue) => {
  const pathQuery = `$.${responseElement}[?(@.${propertyName}=='${expectedValue}')]`;
  const response = dataStoreHelper.getData('response');
...
Then('User verifies response does not contain {string}', (pathQuery) => {
  const response = dataStoreHelper.getData('response');
  const actualValue = utils.getValueOfPropertyFromJson(response, pathQuery);
 ...
 * 
 */
Then('Response body is equal to {json}', (json) => {
  const response = dataStoreHelper.getData('response');
  const diffRes = diff(json, response.body);

  return expect(response.body, `Errors: ${utils.getDeepDiffErrMsg(diffRes)}\n`).to.deep.equal(json);
});

...
const { stringHelper, localizationHelper, dataStoreHelper } = require('ngpd-merceros-testautomation-wdio-ta');
const userData = require('../../data/user-data');
const page = require('../../data/pages-enum');
const fileDataHash = require('../../data/file-data-hash.js');
const jsonReader = require('../../utils/readJsonFile-util').jR;
...
defineParameterType({
  ...
  useForSnippets: false,
  transformer: (string) => {
    if (string.includes('$')) string = stringUtils.replaceAllParamsInString(string);
...
defineParameterType({
  ...
  transformer: (string) => {
    ...
    if (browser.config.params.need_localization) {
      result = localizationHelper.checkLocalization(result);
    }
    result = stringHelper.convertToRegexpIfNeeded(result);

    if (string.startsWith('$')) {
      result = uniqueMap[`${browser.sessionId}${string.slice(1)}`];
    }
...
defineParameterType({
  ...
  transformer: (str) => {
    ...
    if (str.startsWith('$')) {
      return uniqueMap[`${browser.sessionId}${str.slice(1)}`];
    }
  }
});
